...
# import data
trainFile = "latvia_sdm_policy" + policy_train[1:] + ".csv"
testFile = "latvia_sdm_policy" + policy_test[1:] + ".csv"
data_train_val = pd.read_csv("./data/"+trainFile, header=0)
data_test = pd.read_csv("./data/"+testFile, header=0)
all_feature_names = list(data_test.columns)
m, n_total = data_test.shape
...
# name = 'Food.Af Latvia Tot' #'Food.Fc Latvia Tot'#'Food.Food balance2'
# visualize_policy_changes(data_train_val, data_test, name)
...
data_train_val = data_train_val[all_feature_names]
data_test = data_test[all_feature_names]
...
data_train_val.insert(0, "date", dates)
data_test.insert(0, "date", dates)

data_train_val = data_train_val.drop(columns="Months")
data_test = data_test.drop(columns="Months")

data_train_val = data_train_val.set_index('date')
data_test = data_test.set_index('date')


# split test into periods (otherwise use entire 2000-2050 period for testing)
...
# inf values can be replaced with
# data_train_val = data_train_val.replace([np.inf, -np.inf], 0).resample('YE').first()
# data_test = data_test.replace([np.inf, -np.inf], 0).resample('YE').first()


# interpolate data
if interpolate:
    data_train_val = data_train_val.resample(interpolate).interpolate(interpolate_method)
    data_test = data_test.resample(interpolate).interpolate(interpolate_method)
...
# k-fold sampling: training, validation, and test folds
kf = KFold(n_splits=n_folds, shuffle=False)
folds = list(kf.split(data_train_val_scaled))


# visualize training-validation folds
# visualize_folds(folds, n_folds, dates, data_train_val_scaled, data_test_scaled,
#                 all_feature_names, [950,1400,1800])
...
visualize_predictions(policy_train, policy_test,
...
# visualize_eigs(policy_train, policy_test, 
