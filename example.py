...
nc = 5                          # no. of control-input variables
...
rmin, rmax = 1, 15                      # input svd-ranks to test (rmax ~ n*stackmin)
...
n_trials = 500                  # no. of trials with Optuna
...
sampler = 'tpes'                # grid or tpe. grid is fine for small parameter sets. Use tpes (Tree-Structured Parzen Estimator) for large parameter sets

# sensitivity analysis
std_factor = 1.0                # systematic error in control input (fraction of std) after 2020
n_ensemble = 10                 # ensemble size
...
# X_test_forecast, _ = forecast(A_test, B_test, US, r, q, v0_test, u0_test, m_test, u=I_test.to_numpy())
...
# X_train_val_forecast = pd.DataFrame(X_train_val_forecast, columns=state_names).set_index(t_train_val[1:])
# X_test_forecast = pd.DataFrame(X_test_forecast, columns=state_names).set_index(t_test[step:])
# X_test_forecast_neg = pd.DataFrame(X_test_forecast_neg, columns=state_names).set_index(t_test[step:])
# X_test_forecast_pos = pd.DataFrame(X_test_forecast_pos, columns=state_names).set_index(t_test[step:])
...
# re-scale and convert to pandas dataframe
I_train_val = pd.DataFrame(input_scaler.inverse_transform(I_train_val), columns=input_names, index=t_train_val)
X_train_val = pd.DataFrame(state_scaler.inverse_transform(X_train_val), columns=state_names, index=t_train_val)

I_test = pd.DataFrame(input_scaler.inverse_transform(I_test), columns=input_names, index=t_test)
X_test = pd.DataFrame(state_scaler.inverse_transform(X_test), columns=state_names, index=t_test)

X_train_val_forecast = pd.DataFrame(state_scaler.inverse_transform(X_train_val_forecast), columns=state_names, index=t_train_val[1:])
X_test_forecast = pd.DataFrame(state_scaler.inverse_transform(X_test_forecast), columns=state_names, index=t_test[1:])
X_test_forecast_neg = pd.DataFrame(state_scaler.inverse_transform(X_test_forecast_neg), columns=state_names, index=t_test[1:])
X_test_forecast_pos = pd.DataFrame(state_scaler.inverse_transform(X_test_forecast_pos), columns=state_names, index=t_test[1:])
    ...
    for i, ax in enumerate(axes):
        if i<nc:
            ...
            ax.fill_between(X_test_forecast.index, X_test_forecast_neg.iloc[:, i-nc], X_test_forecast_pos.iloc[:, i-nc], alpha=0.2, color=blue,
                            edgecolor='none', label=f'input sensitivity\n ($\\hat u=u\pm{std_factor}\\sigma_u$)')
            # ax.plot(X_test_forecast.iloc[:, i-nc], 's-', color=blue, linewidth=linewidth, label="DMDc", markevery=m_test//8, markersize=1.5)
...
if visualize_AB:
    ...
    im = ax0.imshow(A, aspect='equal', cmap=plt.get_cmap('RdBu'), norm=norm, interpolation='none')
    ax0.set_title('$\\bf\\tilde{A}$', fontsize=fontsize_large) # State transition matrix 
    ax0.yaxis.set_major_locator(MaxNLocator(integer=True))
    ax0.xaxis.set_major_locator(MaxNLocator(integer=True))
    ax0.tick_params(labelsize=fontsize_small)
    
    im = ax1.imshow(B, aspect='equal', cmap=plt.get_cmap('RdBu'), norm=norm, interpolation='none')
...
if visualize_AB:
    ...
    vmin = min(USA.min(), USB.min())
    vmax = min(USA.max(), USB.max())
    norm = TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
    
    fig = plt.figure()
    # gs = GridSpec(1, 3, width_ratios=[2, 0.8, 0.67])
    gs = GridSpec(1, 3, width_ratios=[1, 3, 1.4])
    
    ax0 = plt.subplot(gs[0])
    ax1 = plt.subplot(gs[1])
    ax2 = plt.subplot(gs[2])
    
    im = ax0.imshow(USA, aspect='auto', cmap=plt.get_cmap('RdBu'), norm=norm, interpolation='none')
    ax0.set_title('$\\bf\\tilde{U\\Sigma A}$', fontsize=fontsize_large) # State transition matrix 
    ax0.yaxis.set_major_locator(MaxNLocator(integer=True))
    ax0.xaxis.set_major_locator(MaxNLocator(integer=True))
    ax0.tick_params(labelsize=fontsize_small)
    
    im = ax1.imshow(USB, aspect='auto', cmap=plt.get_cmap('RdBu'), norm=norm, interpolation='none')
    ax1.set_title('$\\bf\\tilde{U\\Sigma B}$', fontsize=fontsize_large) # Control-input input matrix 
    ax1.yaxis.set_major_locator(MaxNLocator(integer=True))
    ax1.xaxis.set_major_locator(MaxNLocator(integer=True))
    ax1.tick_params(labelsize=fontsize_small)
    ...
    fig.colorbar(im, ax=ax2, pad=0, extend='neither', ticks=[-1.5, 0, .5], norm=norm)
    ax2.set_axis_off()
    ax2.set_yticklabels('')
    
    fig.savefig('visualize_USA_USB.svg', bbox_inches='tight')
